...
const HUMANIZED_CONTENT_SCHEMA: JSONSchema = {
  type: 'object',
  properties: {
    humanizedText: {
      type: 'string',
      description: 'The rewritten content that sounds natural and human',
    },
  },
...
Provide the rewritten content in the humanizedText field.`;

  try {
    const result = await aiService.chatStructured<{ humanizedText: string }>(
      [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: userPrompt },
      ],
      HUMANIZED_CONTENT_SCHEMA,
      'humanized_content',
      {
        temperature: 0.7,
        maxTokens: 1500,
    ...
    return result.humanizedText.trim();
  } catch (error) {
