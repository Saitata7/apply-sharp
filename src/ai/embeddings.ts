...
import { djb2Hash } from '@shared/utils/async-utils';
...
/**
 * Create an EmbeddingsService from the user's current AI settings.
 * Returns null if the configured provider doesn't support embeddings
 * (Anthropic and Groq have no embeddings endpoints).
 */
export async function getEmbeddingsService(): Promise<EmbeddingsService | null> {
  try {
    const result = await chrome.storage.local.get('user_settings');
    const settings: AISettings | undefined = result.user_settings?.ai;
    ...
    const service = new EmbeddingsService(settings);
    return service.isAvailable ? service : null;
  } catch (error) {
    console.debug('[EmbeddingsService] Failed to create from settings:', error);
    return null;
  }
}
