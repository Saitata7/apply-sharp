...
// ─── Anti AI Tells (humanization rules, applied to every generation) ──────

/**
 * The single most important set of rules for output that does not get instantly
 * pattern-matched as "ChatGPT slop" by either a human reader or an AI detector.
 *
 * Two reasons this exists:
 *  1. Recruiters skim for 6 seconds. The em-dash + "delve / leverage / tapestry"
 *     vocabulary is now the universal AI fingerprint and gets resumes mentally
 *     discarded before the content is even read.
 *  2. AI-detection bypass tools (Undetectable.ai, StealthGPT, Humbot) are snake
 *     oil per independent benchmarks. The only thing that actually moves both
 *     human and detector reads is prompt-level humanization: persona, controlled
 *     burstiness, banned vocab, occasional fragments.
 *
 * If you change this list, also update src/core/profile/claims-validator.ts
 * detectAITells() so the post-generation linter stays in sync.
 */
export const ANTI_AI_TELLS = `
## Voice Rules  -  These OVERRIDE every other style guideline

### Banned characters
- The em-dash character is FORBIDDEN. Use periods, commas, parentheses, or spaced hyphens.
- The en-dash in prose is also FORBIDDEN.

### Banned vocabulary (zero tolerance)
delve, leverage, tapestry, navigate the complexities, navigate the landscape, robust,
seamless, in today's fast-paced, it's important to note, it is worth noting, furthermore,
moreover, in conclusion, multifaceted, paradigm, embark, foster, holistic, realm,
cutting-edge, unleash, harness, transformative, game-changing, synergy, paradigm shift,
empower, elevate, unlock, journey, passionate about, I am excited to, I believe I would
be a great fit, dive deep, deep dive, hit the ground running.

### Banned structural patterns
- "It's not just X, it's Y" construction.
- "X, Y, and Z" perfectly parallel triads (vary the rhythm or use a different structure).
- Every sentence the same length. Mix 4-word sentences with 25-word ones.
- Every bullet starting with the same syllable count or verb tense.

### Required humanization
- Persona: a tired senior engineer explaining to a peer, NOT a polished marketer.
- Use contractions (I've, didn't, can't) where natural. AI text avoids them; humans do not.
- Allow one deliberate sentence fragment per paragraph for rhythm. Like this.
- Specifics over polish. "Shipped" beats "delivered". "Rewrote" beats "engineered a transformation of".
- One notch less formal than the default. "Hey X" beats "Dear Hiring Manager".

### Inventive use is BANNED
- Never invent metrics, titles, scope, team size, or company facts.
- If you do not have a number, describe scope or complexity instead. Do not guess.
- If a field is unanswerable from the profile and the company research, say so explicitly.
`.trim();
