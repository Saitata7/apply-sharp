...
// ── Workstream 2: getNextQuestion (drives the interview loop) ────────────
...
/**
 * Pick the next question for the conversational profile interview based on
 * the highest-leverage missing dimension. The completeness scorer exposes
 * a nextStep field; this function maps it to a concrete question that the
 * career-advisor persona can ask.
 *
 * The mapping prefers STORY-EXTRACTION questions over field-collection
 * ("walk me through what you actually did at X" beats "what is your
 * phone number") because the agent's creed is "extract stories first,
 * bullets come from stories later."
 */
export interface NextQuestion {
  /** The dimension key the question targets, useful for analytics. */
  dimensionKey: string;
  /** The actual question text the AI should ask. */
  text: string;
  /** Why we are asking this now (shown to the user as a small hint). */
  rationale: string;
  /** Estimated impact: how much this answer will move the completeness score. */
  estimatedImpact: number;
}

const QUESTIONS_BY_DIMENSION: Record<string, { text: string; rationale: string }> = {
  contact: {
    text: 'Quick housekeeping first: what email address and phone number should employers reach you at?',
    rationale: 'Required for any application',
  },
  summary: {
    text: "In one paragraph, how would you describe what you do? Imagine you're explaining your job to a friend, not writing a LinkedIn headline.",
    rationale: 'A summary is the first thing recruiters read',
  },
  primary_experience: {
    text: 'Walk me through what you actually did at your most recent job. What was the broken thing you fixed, what changed because of you, and what was the hardest part?',
    rationale: 'Most impactful single answer for the whole profile',
  },
  experience_depth: {
    text: 'Tell me about another role you held before this one. Same questions: what was broken, what did you do, what changed?',
    rationale: 'A second story shows trajectory and depth',
  },
  skills: {
    text: 'List the top 5 to 8 technologies you would actually be comfortable being interviewed on tomorrow. Not "have heard of", "could deeply explain".',
    rationale: 'Honest skills are more useful than padded skills',
  },
  education: {
    text: 'What is your highest degree or most recent certification? Include the school and year.',
    rationale: 'Some roles still gate on education',
  },
  projects: {
    text: "Tell me about a side project, open source contribution, or work project you're proud of. What did you build and why does it matter?",
    rationale: 'Projects often tell a sharper story than work history',
  },
  defensibility: {
    text: 'Pick your strongest accomplishment from the bullets we have so far. Can you put a number on it? Time saved, money earned, users reached, errors prevented, anything?',
    rationale: 'Defensible numbers beat vague claims',
  },
};

export function getNextQuestion(profile: MasterProfile): NextQuestion | null {
  const report = scoreProfileCompleteness(profile);
  if (!report.nextStep) return null;

  const key = report.nextStep.key;
  const question = QUESTIONS_BY_DIMENSION[key];
  ...
  return {
    ...
    estimatedImpact: report.nextStep.weight - report.nextStep.score,
  };
}
